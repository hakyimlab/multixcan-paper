...
(function(){
  mt_30_ <- "data/simulations/combination_all_1kr__ccn30_cc5__mt_results.txt" %>% load_mt_("multixcan", "condition number 30") %>% select(gene, pvalue, label, n_models)
  mt_0_ <- "data/simulations/combination_all_1kr__ccn0_cc5__mt_results.txt" %>% load_mt_("multixcan", "None") %>% select(gene, pvalue, label, n_models)

  d <- rbind(mt_30_, mt_0_) %>% mutate(s = -log10(pvalue))

  p <- ggplot(d) + geom_smooth(aes(x=n_models, y=s, color=label)) +
    paper_plot_theme_a() + xlab("Number of tissues") + ylab(expression(Significance~(-log10(pvalue)))) +
    ggtitle("Effect of PCA Regularization", subtitle="Simulated traits using\n all available tissues for every gene") + 
    guides(color=guide_legend(title="Regularization"))
  p %>% save_png(fp_("combination_all_tendency.png"), 800, 800)
})()
```
...
(function(){
  ...
  p <- the_box_plot_(d) + xlab("Condition number") + ylab(expression(Significance~(-log10(pvalue)))) + 
    ggtitle("Significance") + facet_wrap(~the_facet, ncol=1, scales="free")
  p %>% save_png(fp_("pc.png"), 800, 800)
})()
```