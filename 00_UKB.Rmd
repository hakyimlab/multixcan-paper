...
(function(){
  d <- mt_significant %>% filter(n_multixcan_significant > 0 | n_predixcan_significant > 0)
  ...
  m_none <- mt_significant %>% filter(n_multixcan_significant ==0, n_predixcan_significant == 0) %>% nrow()
  m_how_many <- d %>% mutate(h = n_multixcan_significant - n_predixcan_significant) %>% .$h %>% mean()
  ...
  r <- data.frame(field=c("MulTixcan wins", "Ties", "PrediXcan wins", "No detection", "MulTixcan Average Improvement", "Average overlap"),
