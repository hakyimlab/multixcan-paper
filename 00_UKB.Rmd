...
(function() {
  ...
  mtp %>%  p_save_delim(file.path(results_dir, "ukb_high_cholesterol_tissuexcan_significant.txt"))
...
(function(){
  ...
  (do_scatter_plot_mt_vs_smt (names_c_) + 
    ...
    facet_wrap(~phenotype, scales="fixed")) %>% save_png(file.path(plots_dir, "smt_vs_mt_ukb.png"), height=400, width=900)
  ...
  selected_mt_vs_smt <- function(names, selected, title, subtitle, path) {
    ...
    (do_scatter_plot_mt_vs_smt (names_) + ggtitle(title, subtitle=subtitle) + facet_wrap(~phenotype, scales="free", ncol =3)) %>% save_png(path, height=1000, width=1000)
  }
...
p_save_delim(stats, file.path(results_dir, "ukb_tissuexcan_stats.txt"))
...
p_save_delim(mt_significant, file.path(results_dir, "ukb_tissuexcan_significant.txt"))
```