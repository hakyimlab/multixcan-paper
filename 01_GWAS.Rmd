...
stats <- pheno_info %>% select(tag, consortium, name) %>% inner_join(analisis_results_q$stats, by="tag")
p_save_delim(stats, file.path(results_dir, "gwas_smultixcan_stats_q.txt"))

smt_significant <- pheno_info %>% select(tag, consortium, name) %>% 
  inner_join(analisis_results_q$smt_significant, by=c("tag"="phenotype")) %>%
  select(-eigen_max, -eigen_min, -eigen_min_kept, -z_min, -z_max, -z_mean, -z_sd, -tmi, -status)
p_save_delim(smt_significant, file.path(results_dir, "gwas_smultixcan_significant_q.txt"))
...
(function(){
  d <- smt_significant %>% filter(n_smultixcan_significant > 0 | n_spredixcan_significant > 0)
  ...
  m_none <- smt_significant %>% filter(n_smultixcan_significant ==0, n_spredixcan_significant == 0) %>% nrow()
  m_how_many <- d %>% mutate(h = n_smultixcan_significant - n_spredixcan_significant) %>% .$h %>% mean()
  ...
  r <- data.frame(field=c("S-MulTixcan wins", "Ties", "S-PrediXcan wins", "No detection", "S-MulTixcan Average Improvement", "Average overlap"),
