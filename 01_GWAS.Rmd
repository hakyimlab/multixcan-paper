...
p_save_delim(stats, file.path(results_dir, "gwas_summary_crosstissuexcan_stats.txt"))
...
sp_significant <- pheno_info %>% select(tag, consortium, name) %>% 
  ...
  select(-p_smr,-p_heidi,-twas_pvalue,-p_h0, -p_h1, -p_h2, -p_h3, -p_h4) %>% rename(pvalue=pval)
...
p_save_delim(smt_significant, file.path(results_dir, "gwas_summary_crosstissuexcan_significant.txt"))
